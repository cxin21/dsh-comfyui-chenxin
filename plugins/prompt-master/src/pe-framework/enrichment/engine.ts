 ...
 * 负向推断/角色锚点补全），注入 CINEMA_LEXICON 关键类目作为候选词；经 complete 让 LLM 只输出
 * 增量 JSON patch（{set, additions, expansions}，兼容裸部分蓝图 {core:…} 形态）；应用 patch 到 v0 → v1；
 * 风格注入（opts.styleId → applyStyle）；expansions 从 LLM 收集（无则用 opts.missing 生成占位记录）；
 * patch 应用失败 → 回退 v0 + expansions:['enrichment_failed:fallback_to_v0']（不抛错）；
 * v1 产出后必须过 checkConcreteness + checkFidelity——任一失败追加 advisory 进 expansions 但不阻断。
 */
...
export async function enrichBlueprint(
  ...
  try {
    ...
    // 质量自检：具体性 + 保真（失败追加 advisory 进 expansions，不阻断）
